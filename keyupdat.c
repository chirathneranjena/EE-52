...
/*
   ...
   Description:      This function handles the <Track Up> key when nothing is
                     happening in the system.  It increments the track number,
                     resetting the track time and loading the track
...
{
    /* variables */
    int  track;         /* new track number (for efficiency) */



    /* increment and display the track number */
    track = update_track_no(+1);
    display_track(track + 1);
...
/*
   ...
   Description:      This function handles the <Track Down> key when nothing
                     is happening in the system.  It just decrements the track
                     number, resetting the track time and loading the track
                     information for the new track.
...
enum status  do_TrackDown(enum status cur_status)
{
    ...
    /* decrement and display the track number */
    track = update_track_no(-1);
    display_track(track + 1);
